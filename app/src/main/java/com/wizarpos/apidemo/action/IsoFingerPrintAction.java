...
public class IsoFingerPrintAction extends ActionModel {
    ...
    private static final int ISOFINGERPRINT_TYPE_DEFAULT = 0;
    private static final int ISOFINGERPRINT_TYPE_ISO2005 = 1;
    private static final int ISOFINGERPRINT_TYPE_ISO2015 = 2;
    ...
    public void enroll(Map<String, Object> param, ActionCallback callback) {
        try {
            callback.sendResponse(mContext.getString(R.string.press_fingerprint));//请按手指
            userID++;
            ...
            device.listenForEnroll(new OperationListener() {
                ...
                public void handleResult(OperationResult operationResult) {
                    if (operationResult instanceof FingerprintPressOperationResult) {
                        ...
                        callback.sendResponse(mContext.getString(R.string.remove_fingerprint));//请移开手指
                        ...
                        callback.sendResponse(Constants.HANDLER_LOG_FAILED, mContext.getString(R.string.enroll_timeout));//超时
                        ...
                        callback.sendResponse(Constants.HANDLER_LOG_SUCCESS, "finger.length=" + fingerprint.getFeature().length);
                    }
                }
            }, 10000);
        } catch (DeviceException e) {
    ...
    public void identify(Map<String, Object> param, ActionCallback callback) {
        try {
            ...
            callback.sendResponse(mContext.getString(R.string.press_fingerprint) + "1");//请按手指
            fpList.add(device.getFingerprint(ISOFINGERPRINT_TYPE_ISO2005).getFeature());
            callback.sendResponse(mContext.getString(R.string.press_fingerprint) + "2");//请按手指
            fpList.add(device.getFingerprint(ISOFINGERPRINT_TYPE_ISO2005).getFeature());
            callback.sendResponse(mContext.getString(R.string.press_fingerprint) + "3");//请按手指
            fpList.add(device.getFingerprint(ISOFINGERPRINT_TYPE_ISO2005).getFeature());
            callback.sendResponse(mContext.getString(R.string.press_fingerprint) + "4");//请按手指
            fpList.add(device.getFingerprint(ISOFINGERPRINT_TYPE_ISO2005).getFeature());
